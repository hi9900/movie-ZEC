...
export default {
  ...
  data() {
    return{
      ...
      user_email_rule: [
        v => !!v || '이메일는 필수 입력사항입니다.',
        // 이메일 생긴거 확인하기
        v => /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]{2,3}$/.test(v.replace(/(\s*)/g, '')) || '이메일 형식으로 입력해주세요',
      ],
      // 이름 확인
      username: '',
      username_rule: [
        v => !!v || '이름은 필수 입력사항입니다.',
        v => !(v && v.length >= 11) || '2~10자의 문자 및 숫자만 사용 가능합니다.',
        v => !(v && v.length <= 1) || '2~10자의 문자 및 숫자만 사용 가능합니다.',
        v => !/[~!@#$%^&*()_+|<>?:{}]/.test(v) || '이름에는 특수문자를 사용할 수 없습니다.',
      ],
      // 비밀번호 유효성 나중에 하기
      password1: '',
      password_rule: [
        v => this.password1 === 'ins' ? !!v || '패스워드는 필수 입력사항입니다.' : true,
        v => !(v && v.length >= 16) || '8~16자 영문 대 소문자, 숫자, 특수문자를 사용하세요',
  ...
  methods: {
    ...
    if (this.username) {
      try {
        const response = await axios.get(`${API_URL}/accounts/check-username/${this.username}/`);
